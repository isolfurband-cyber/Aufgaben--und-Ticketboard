...
st.sidebar.markdown("---")
st.sidebar.subheader("Nächste Termine")

if not st.session_state.termine:
  st.sidebar.info("Keine Termine vorhanden.")
else:
  sorted_termine = sorted(st.session_state.termine, key=lambda x: x["datum"])
  ...
  for idx, term in enumerate(sorted_termine):
    ...
    # Farbliche Kennzeichnung: Vergangen = Rot, Zukunft/Heute = Standard-Sidebar-Box
    bg_color = "rgba(255, 75, 75, 0.2)" if is_past else "rgba(255, 255, 255, 0.05)"
    ...
    if st.sidebar.button("🗑️ Löschen", key=f"del_term_{term['id']}_{idx}"):
      st.session_state.termine = [
          item for item in st.session_state.termine if item["id"] != term["id"]
      ]
      save_termine(st.session_state.termine)
...
if menu == "🚪 Abmelden":
  ...
  with st.form("new_ticket_form"):
    ...
    with col1:
      ...
      objekt = st.text_input("Objekt / Adresse", "Talstr. 32, 07545 Gera")
    ...
    with col2:
      ...
      faelligkeitsdatum = st.date_input("Fälligkeitsdatum", datetime.now())
    ...
    submit_ticket = st.form_submit_button(label="Ticket im System speichern")
      ...
      if not titel:
        ...
        if uploaded_files:
          for file in uploaded_files:
            ...
            anhänge.append(
                {"name": file.name, "data": b64_encoded, "type": file.type}
            )
  ...
  with col_f1:
    filter_status = st.selectbox(
        "Nach Status filtern", ["Alle", "Offen", "In Bearbeitung", "Erledigt"]
    )
  with col_f2:
  ...
  if not tickets:
    ...
    for idx, t in enumerate(reversed(tickets)):
      ...
      with st.expander(
          f"{prio_color} [#{t.get('id', 0)}] {t.get('titel', 'Unbenannt')} —"
          f" Objekt: {t.get('objekt', '')} ({t.get('status', 'Offen')})"
      ):
        ...
        if st.session_state[edit_key]:
          ...
          with col_b1:
            if st.button(
                "💾 Ändern speichern", key=f"save_desc_{t.get('id', 0)}"
            ):
              ...
              st.rerun()
          with col_b2:
            if st.button("Abbrechen", key=f"cancel_desc_{t.get('id', 0)}"):
              st.session_state[edit_key] = False
              st.rerun()
        else:
          st.markdown(f"> {t.get('beschreibung', '')}")
          if st.button(
              "✏️ Beschreibung bearbeiten", key=f"btn_edit_{t.get('id', 0)}"
          ):
        ...
        if t.get("anhaenge"):
          for f_idx, anhang in enumerate(t["anhaenge"]):
            ...
            with col_fv:
              ...
              if st.button(
                  "👁️ Ansehen", key=f"btn_prev_{t.get('id', 0)}_{f_idx}"
              ):
            ...
            with col_fd:
              if st.button(
                  "🗑️ Löschen", key=f"del_file_{t.get('id', 0)}_{f_idx}"
              ):
            ...
            if st.session_state.get(f"show_{t.get('id', 0)}_{f_idx}", False):
              if "image" in anhang["type"] or anhang["name"].lower().endswith(
                  (".png", ".jpg", ".jpeg")
              ):
                st.image(
                    file_bytes,
              ...
              elif "pdf" in anhang["type"] or anhang["name"].lower().endswith(
                  ".pdf"
              ):
                ...
                st.download_button(
                    ...
                    mime=anhang["type"],
                    key=f"dl_fallback_{t.get('id', 0)}_{f_idx}",
                )
        else:
        ...
        with st.form(key=f"add_more_form_{t.get('id', 0)}"):
          ...
          add_more_btn = st.form_submit_button("Dateien zum Ticket hochladen")
          if add_more_btn and more_files:
            ...
            for file in more_files:
              ...
              t["anhaenge"].append(
                  {"name": file.name, "data": b64_enc, "type": file.type}
              )
            save_tickets(st.session_state.tickets)
        ...
        with col_act1:
          neuer_status = st.selectbox(
              ...
              index=["Offen", "In Bearbeitung", "Erledigt"].index(
                  t.get("status", "Offen")
              ),
        ...
        with col_act2:
          if st.button(
              "🗑️ Ticket komplett löschen", key=f"del_{t.get('id', 0)}_{idx}"
          ):
